                        ...
                        >
                            ...
                            <div className="space-y-4">
                                ...
                                <button
                                    ...
                                    className="w-full flex items-center justify-between p-4 bg-zinc-900/50 border border-zinc-800 rounded-2xl hover:bg-blue-600/10 hover:border-blue-500/50 transition-all text-left"
                                >
                                    <div>
                                        <p className="text-sm font-bold text-white">Interface WiFi (Standard)</p>
                                        <p className="text-[10px] text-zinc-500 font-mono italic">TCP: 192.168.0.10:35000</p>
                                    </div>
                                ...
                                {/* USB OTG Option */}
                                <button
                                    onClick={() => alert("Conecta tu cable USB-OTG y da permiso en Android.")}
                                    className="w-full flex items-center justify-between p-4 bg-zinc-900/50 border border-zinc-800 rounded-2xl hover:bg-amber-600/10 hover:border-amber-500/50 transition-all text-left"
                                >
                                    <div>
                                        <p className="text-sm font-bold text-white">Cable USB / VAG-COM</p>
                                        <p className="text-[10px] text-zinc-500 font-mono italic">Conexión por cable (Más estable)</p>
                                    </div>
            ...
            <AnimatePresence mode="wait">
                ...
                {activeTab === 'analysis' && (
                    <motion.div
                        ...
                        className="p-4"
                    >
                        <ComparisonDashboard liveData={obdData} />
                    </motion.div>
                )}
                                ...
                                history.map((report) => (
                                    <div key={report.id} className="bg-zinc-900/50 border border-zinc-800 p-5 rounded-2xl">
                                        <div className="flex justify-between items-center mb-3">
                                            <span className="text-[10px] font-mono text-zinc-500 uppercase">{new Date(report.fecha).toLocaleDateString()}</span>
                                            {report.errores_dtc?.length > 0 && <span className="text-[8px] bg-red-500 text-white font-bold px-2 py-0.5 rounded-full">ALERTA</span>}
                                        </div>
                                        <p className="text-sm font-bold text-zinc-200 mb-2 truncate">{report.resumen}</p>
                                        <div className="flex gap-6 mt-2 opacity-60">
                                            <div className="flex items-center gap-1">
                                                <Activity size={10} className="text-blue-400" />
                                                <span className="text-[10px] font-bold">{report.voltaje_bateria}V</span>
                                            </div>
                                            <div className="flex items-center gap-1">
                                                <Activity size={10} className="text-emerald-400" />
                                                <span className="text-[10px] font-bold">{report.carga_motor}%</span>
                                            </div>
                                        </div>
                                    </div>
                                ))
